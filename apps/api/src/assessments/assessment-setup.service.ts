...
@Injectable()
export class AssessmentSetupService extends AssessmentDomainService {
  ...
  async listAssessmentTypes(query: LookupQueryDto = {}) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.db.assessmentType.findMany({
        where,
        orderBy: { code: "asc" },
      ...
      this.prisma.db.assessmentType.count({ where })
    ]);
  ...
  async createAssessmentType(input: CreateLookupDto, actorUserId: string) {
    ...
    const record = await this.prisma.db.assessmentType.create({
      data: { code, name: requireText(input.name, "name") },
      select: lookupSelect
    });
    await this.audit(actorUserId, "assessments.assessmentType.created", "assessmentType", record.id);
    return record;
  }

  async updateAssessmentType(id: string, input: UpdateLookupDto, actorUserId: string) {
    const record = await this.prisma.db.assessmentType.update({
      where: { id },
      data: {
    ...
    await this.audit(actorUserId, "assessments.assessmentType.updated", "assessmentType", record.id);
    return record;
  }
  ...
  async listLevels(query: LookupQueryDto = {}) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.db.assessmentLevel.findMany({
        where,
      ...
      this.prisma.db.assessmentLevel.count({ where })
    ]);
  ...
  async createLevel(input: CreateLookupDto, actorUserId: string) {
    ...
    const record = await this.prisma.db.assessmentLevel.create({
      data: {
        code,
    ...
    await this.audit(actorUserId, "assessments.level.created", "assessmentLevel", record.id);
    return record;
  }

  async updateLevel(id: string, input: UpdateLookupDto, actorUserId: string) {
    const record = await this.prisma.db.assessmentLevel.update({
      where: { id },
      data: {
        ...(input.name !== undefined ? { name: requireText(input.name, "name") } : {}),
        ...(input.status !== undefined ? { status: enumValue(input.status, ["active", "inactive", "archived"], "status") } : {}),
        ...(input.sortOrder !== undefined ? { sortOrder: nullableNumber(input.sortOrder, "sortOrder") } : {})
      } as never,
      select: levelSelect
    });
    await this.audit(actorUserId, "assessments.level.updated", "assessmentLevel", record.id);
    return record;
  }
  ...
  async listAssessments(query: ListAssessmentsQueryDto = {}) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.db.assessment.findMany({
        where,
      ...
      this.prisma.db.assessment.count({ where })
    ]);
  ...
  async getAssessment(id: string) {
    const assessment = await this.prisma.db.assessment.findUnique({ where: { id }, select: assessmentSelect });
  ...
  async createAssessment(input: CreateAssessmentDto, actorUserId: string) {
    ...
    const assessment = await this.prisma.db.assessment.create({
      data: {
        ...
        termId: requireText(input.termId, "termId"),
    ...
    await this.audit(actorUserId, "assessments.assessment.created", "assessment", assessment.id);
    return assessment;
  }
  ...
  async updateAssessment(id: string, input: UpdateAssessmentDto, actorUserId: string) {
    ...
    const assessment = await this.prisma.db.$transaction(async (tx) => {
      ...
      return tx.assessment.update({
        ...
        data: {
          ...
          ...(input.termId !== undefined ? { termId: requireText(input.termId, "termId") } : {}),
    ...
    await this.audit(actorUserId, "assessments.assessment.updated", "assessment", assessment.id);
    return assessment;
  }

  async archiveAssessment(id: string, actorUserId: string) {
    const assessment = await this.prisma.db.assessment.update({
      where: { id },
      data: { status: "archived" },
      select: assessmentSelect
    });
    await this.audit(actorUserId, "assessments.assessment.archived", "assessment", assessment.id);
    return assessment;
  }

  async publishAssessment(id: string, actorUserId: string) {
    const questionCount = await this.prisma.db.assessmentQuestion.count({ where: { assessmentId: id } });
    if (questionCount === 0) {
    ...
    await this.audit(actorUserId, "assessments.assessment.published", "assessment", assessment.id);
    return assessment;
  }

  private async assertAssessmentTypeCodeAvailable(code: string): Promise<void> {
    const existing = await this.prisma.db.assessmentType.findUnique({ where: { code }, select: { id: true } });
    if (existing) {
  ...
  private async assertLevelCodeAvailable(code: string): Promise<void> {
    const existing = await this.prisma.db.assessmentLevel.findUnique({ where: { code }, select: { id: true } });
    if (existing) {

...
export type PrismaAccess = Pick<PrismaService, "db">;
...
export abstract class AssessmentDomainService {
  protected constructor(protected readonly prisma: PrismaAccess) {}

  protected async audit(actorUserId: string, event: string, targetType: string, targetId: string): Promise<void> {
...
export function normalizePagination(query: PaginationQuery): { page: number; pageSize: number; skip: number } {
  const page = clampNumber(query.page, 1, 1, 100_000);
  const pageSize = clampNumber(query.pageSize, 25, 1, 100);
  return { page, pageSize, skip: (page - 1) * pageSize };
}

export function toPage<T>(items: T[], total: number, pagination: { page: number; pageSize: number }): Paginated<T> {
...
export function autoMarkResponse(
  ...
  if ((question.questionType === "short_answer" || question.questionType === "numeric") && question.correctAnswer) {
    const isCorrect = responseText?.trim().toLowerCase() === question.correctAnswer.trim().toLowerCase();
